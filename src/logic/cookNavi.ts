 ...
 *
 * 手順の分類:
 *   - 「待ち系」= 煮る・焼く（蒸し焼き）・蒸す・漬ける・炊く・冷ます・寝かせる 等の
 *      “その間は手が空く”動詞を含み、かつ minutes（待ち分数）が設定されている手順
 *   - 「手作業系」= それ以外（切る・こねる・炒める・揚げる など、手を動かし続ける工程）
 *
 ...
 *
 * 設計判断: 炒める・揚げるは “時間が書いてあっても手を動かし続ける/目を離せない” 工程なので
 * あえて辞書に入れない（ロードマップが列挙する待ち動詞にも含まれていない）。
 * これにより「3分炒める」「4分揚げる」は手作業系のままになる。
 * 一方「8分蒸し焼き」「10分おく」「15分漬け込む」などは待ち系として拾える。
 */
export const WAIT_VERB_PATTERNS: RegExp[] = [
  /煮/, // 煮る・煮込む・煮からめる
  /焼/, // 焼く（主に「蒸し焼き」。焼き色を付けるだけの工程は minutes 無しなので手作業系のまま）
  /蒸/, // 蒸す・蒸らす・蒸し焼き
  /漬/, // 漬ける・漬け込む
  /炊/, // 炊く
  /茹で|ゆで/, // 茹でる
